...
class _RecipeDetailScreenState extends State<RecipeDetailScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadDifficultyOptions() async {
    try {
      final categories = await _categoryService.getCategoriesByType('difficulty');
      if (categories != null && categories.isNotEmpty) {
        setState(() {
          _difficultyOptions = categories.map((e) => e.name).toList();
          // Ensure default value is valid
          if (!_difficultyOptions.contains(_difficulty) && _difficultyOptions.isNotEmpty) {
    ...
    try {
      ...
      if (recipe != null) {
        setState(() {
          ...
          _timeController = TextEditingController(text: recipe.time.replaceAll(RegExp(r'[^0-9]'), ''));
    ...
    Future<void> _saveRecipe() async {
      ...
      if (_nameController.text.trim().isEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          ...
          const SnackBar(content: Text('请输入菜谱名称'), duration: Duration(seconds: 1)),
      ...
      try {
        ...
        if (widget.isCreateMode) {
          ...
          result = await _recipeService.updateRecipe(widget.recipeId!, recipeData);
        ...
        if (mounted) {
          ...
          if (result != null) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              ...
              const SnackBar(content: Text('保存失败，请重试'), duration: Duration(seconds: 1)),
          ...
          child: Row(
            ...
            children: [
              Container(
                ...
                child: Text(
                  "${index + 1}",
                  style: GoogleFonts.nunito(fontSize: 12, fontWeight: FontWeight.bold, color: _sageGreen),
                ),
              ),
              ...
              Expanded(
                child: _isEditing
                  ? MinimalInput(
                      ...
                      hintText: "描述这个步骤...",
                      style: const TextStyle(fontSize: 14, height: 1.4, color: _textPrimary),
              ...
              if (_isEditing) ...[
                ...
                const Icon(Icons.drag_handle, color: Color(0xFFE0E0E0), size: 18),
              ],
            ],
          ),
        ),
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 8),
                        Text("烹饪时长", style: const TextStyle(fontSize: 14, color: _textSecondary)),
                        const Spacer(),
                        _isEditing 
                          ? SizedBox(
                              width: 80,
                              child: MinimalInput(
                                ...
                                style: GoogleFonts.nunito(fontSize: 16, fontWeight: FontWeight.bold, color: _textPrimary),
                              ),
                            )
                          : Text(
                        ...
                        Text("分钟", style: GoogleFonts.nunito(fontSize: 14, color: _textSecondary)),
                      ],
                    ),
                    ...
                    Column(
                      ...
                      children: [
                         Row(
                           children: [
                             const Icon(Icons.bar_chart_rounded, size: 18, color: _textSecondary),
                             const SizedBox(width: 8),
                             Text("难度等级", style: const TextStyle(fontSize: 14, color: _textSecondary)),
                           ],
                         ),
                        ...
                        Wrap(
                          ...
                          children: _difficultyOptions.map((option) {
                            ...
                            return GestureDetector(
                              onTap: _isEditing ? () => setState(() => _difficulty = option) : null,
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  const Text("食材清单", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: _textPrimary)),
                  if (_isEditing)
                    RotationTransition(
                      turns: Tween(begin: 0.0, end: 0.25).animate(CurvedAnimation(parent: _rotateController, curve: Curves.elasticOut)),
                      child: GestureDetector(
                        onTap: _addIngredient,
                        child: const Padding(
                          padding: EdgeInsets.all(4.0),
                          child: Icon(Icons.add_circle, color: _sageGreen, size: 24),
                        ),
                      ),
                    ),
                ],
                    ...
                    return Column(
                      children: [
                        Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                ...
                                child: _isEditing
                                  ? MinimalInput(
                                      ...
                                      onChanged: (val) => _ingredients[index] = Ingredient(name: val, amount: _ingredients[index].amount),
                                    )
                                  : Text(
                              ...
                              // Vertical Divider
                              Container(height: 16, width: 1, color: Colors.grey.withOpacity(0.2), margin: const EdgeInsets.symmetric(horizontal: 8)),
                              // Amount (30%)
                              Expanded(
                                flex: 3,
                                child: _isEditing
                                  ? MinimalInput(
                                      ...
                                      hintText: "用量",
                                      textAlign: TextAlign.end,
                                      style: GoogleFonts.nunito(fontSize: 15, color: _textSecondary),
                                      onChanged: (val) => _ingredients[index] = Ingredient(name: _ingredients[index].name, amount: val),
                                    )
                                  : Text(
                              ...
                              if (_isEditing) ...[
                                ...
                                GestureDetector(
                                  ...
                                  child: Icon(Icons.remove_circle_outline, color: Colors.grey.withOpacity(0.4), size: 18),
                                ),
                              ],
                            ],
                          ),
                        ...
                        if (index < _ingredients.length - 1)
                          const Divider(height: 1, indent: 16, color: Color(0xFFFAFAFA)),
                      ],
                    );
                  }),
                ),
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text("烹饪步骤", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: _textPrimary)),
                  if (_isEditing)
                    GestureDetector(
                      onTap: _addStep,
                      child: const Padding(
            ...
            children: [
              Expanded(
                ...
                child: OutlinedButton(
                  ...
                  child: Text(_isEditing && !widget.isCreateMode ? "取消" : "返回", style: const TextStyle(fontSize: 15)),
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                ...
                child: ElevatedButton(
                  ...
                  child: Text(_isEditing ? "保存菜谱" : "编辑菜谱", style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
                ),
              ),
            ],
          ),
