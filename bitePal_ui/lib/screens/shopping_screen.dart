...
class BouncyCard extends StatefulWidget {
  ...
  const BouncyCard({super.key, required this.child, this.onTap, this.isSelected = false});
...
class _ShoppingScreenState extends State<ShoppingScreen> with RefreshableScreenState<ShoppingScreen> {
  ...
  Future<void> _markAsPurchased(ShoppingItem item) async {
    ...
    await _shoppingService.updateShoppingItem(_currentList!.id, item.id, checked: true, actualAmount: actualAmount);
    setState(() { _items.removeWhere((i) => i.id == item.id); });
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('"${item.name}" 已放入菜篮 🧺'), duration: const Duration(seconds: 1)));
  }
      ...
      builder: (context) => AddShoppingItemDialog(
        onAdd: (name, amount) async {
          ...
          if (_currentList != null) {
            await _shoppingService.addShoppingItem(_currentList!.id, name: name, amount: amount, price: 0);
            _loadShoppingList();
          }
        },
      ),
      ...
      body: CustomScrollView(
        slivers: [
          ...
          SliverAppBar(
            ...
            title: const Text("购物清单", style: TextStyle(fontWeight: FontWeight.w800, color: _textPrimary)),
            actions: [
              IconButton(
                icon: const Icon(Icons.history_rounded, color: _textPrimary),
                onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ShoppingHistoryScreen())),
              ),
                ...
                child: GlassContainer(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Expanded(
                        child: TextField(
                          decoration: InputDecoration(hintText: "搜索清单项目...", border: InputBorder.none, isDense: true),
  ...
  Widget _buildShoppingItem(ShoppingItem item) {
    return Padding(
      ...
      child: Dismissible(
        ...
        background: _buildDismissBackground(Alignment.centerLeft, Icons.check_circle_rounded, _sageGreen),
        secondaryBackground: _buildDismissBackground(Alignment.centerRight, Icons.delete_outline_rounded, _persimmon),
        onDismissed: (dir) {
          ...
          child: Container(
            ...
            child: Row(
              children: [
                Container(
                  width: 24, height: 24,
                  decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: _sageGreen.withOpacity(0.3), width: 2)),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(item.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: _textPrimary)),
                      if (item.amount.isNotEmpty)
                        Text("需要: ${item.amount}", style: const TextStyle(fontSize: 12, color: _textSecondary)),
                    ],
                  ),
                ),
  ...
  Widget _buildModernFAB() {
    return GlassContainer(
      ...
      child: Row(
        ...
        children: [
          _buildFABAction(Icons.add_rounded, "添加食材", _sageGreen, _showAddItemDialog),
          const SizedBox(width: 8),
          _buildFABAction(Icons.shopping_bag_rounded, "去结算", _persimmon, () {
            if (_currentList != null) {
              Navigator.push(context, MaterialPageRoute(builder: (_) => PurchasedItemsScreen(listId: _currentList!.id))).then((v) {
      ...
      child: Column(
        ...
        children: [
          TweenAnimationBuilder<double>(
            ...
            child: Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: _sageGreen.withOpacity(0.1), shape: BoxShape.circle), child: const Icon(Icons.shopping_basket_outlined, size: 48, color: _sageGreen)),
          ),
          const SizedBox(height: 24),
          const Text("清单已清空", style: TextStyle(color: _textSecondary, fontSize: 16, fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
          const Text("所有的美味都已准备就绪 ~", style: TextStyle(color: Color(0xFFDCD7CD), fontSize: 14)),
        ],
      ),
    );
  }
