...
type IngredientItem struct {
	...
	IngredientID string         `json:"ingredientId" gorm:"type:varchar(36);index"`                      // 关联食材库
	Quantity     float64        `json:"quantity" gorm:"type:decimal(10,2)"`                             // 数量
	UnitID       string         `json:"unitId" gorm:"type:varchar(20);index"`                           // 单位ID，须为该食材的 ingredient_unit
	Name         string         `json:"name" gorm:"type:varchar(100);index"`                             // 兼容旧数据，新数据以 Master 为准
	Unit         string         `json:"unit" gorm:"type:varchar(20)"`                                    // 兼容旧数据
	Amount       string         `json:"amount" gorm:"type:varchar(50)"`                                  // 兼容旧数据
	Storage      string         `json:"storage" gorm:"type:varchar(36);index"`                          // 存储位置ID（StorageLocation的ID）
	CategoryID   string         `json:"categoryId" gorm:"type:varchar(36);index"`                        // 食材类型分类ID
	...
	Master   *IngredientMaster   `json:"master,omitempty" gorm:"foreignKey:IngredientID"`  // 食材库
	UnitRef  *Unit               `json:"unitRef,omitempty" gorm:"foreignKey:UnitID;references:ID"` // 单位
}
...
// formatAmount 根据数量与单位生成描述
func formatAmount(qty float64, unit string) string {
	if qty == float64(int(qty)) {
		return fmt.Sprintf("%d%s", int(qty), unit)
	}
	return fmt.Sprintf("%.2f%s", qty, unit)
}

// IngredientResponse 食材响应结构（包含计算字段）
type IngredientResponse struct {
	ID            string                  `json:"id"`           // 食材ID
	IngredientID  string                  `json:"ingredientId"`  // 食材库ID（单位统一化）
	UnitID        string                  `json:"unitId"`        // 单位ID（单位统一化）
	Name          string                  `json:"name"`           // 食材名称（展示）
	Quantity      float64                 `json:"quantity"`      // 数量数值
	Unit          string                  `json:"unit"`           // 单位（展示名称）
...
func (i *IngredientItem) ToResponse() *IngredientResponse {
	...
	// 展示名称与单位：优先用食材库与单位表，兼容旧数据
	name, unit, amount := i.Name, i.Unit, i.Amount
	if i.IngredientID != "" && i.Master != nil {
		name = i.Master.Name
	}
	if i.UnitID != "" && i.UnitRef != nil {
		unit = i.UnitRef.DisplayName
	}
	if amount == "" && i.Quantity > 0 && unit != "" {
		amount = formatAmount(i.Quantity, unit)
	}
