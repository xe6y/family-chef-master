...
// RecipeIngredientDisplay 菜谱食材展示（含解析后的名称、用量、库存状态）
type RecipeIngredientDisplay struct {
	IngredientID string   `json:"ingredientId"`
	Quantity     *float64 `json:"quantity"`
	UnitID       string   `json:"unitId"`
	Name         string   `json:"name"`
	AmountDisplay string  `json:"amountDisplay"` // 如 "鸡腿 2只"、"盐 适量"
	Status       string   `json:"status"`         // sufficient / insufficient / unknown
}
...
// enrichRecipeDetail 富化菜谱详情：解析食材名称/用量、库存状态、总热量
func (h *RecipeHandler) enrichRecipeDetail(ingredients []models.RecipeIngredient, userID, familyID string) (
	display []RecipeIngredientDisplay,
	caloriesTotal, proteinTotal, fatTotal, carbTotal float64,
) {
	if len(ingredients) == 0 {
		return nil, 0, 0, 0, 0
	}
	display = make([]RecipeIngredientDisplay, 0, len(ingredients))
	for _, ing := range ingredients {
		item := RecipeIngredientDisplay{
		...
		nameOK := config.DB.First(&master, "id = ?", ing.IngredientID).Error == nil
		unitOK := config.DB.First(&unit, "id = ?", ing.UnitID).Error == nil
		if nameOK {
			item.Name = master.Name
		}
		if unitOK {
			if ing.Quantity != nil && *ing.Quantity > 0 {
				item.AmountDisplay = fmt.Sprintf("%.0f%s", *ing.Quantity, unit.DisplayName)
				if *ing.Quantity != float64(int(*ing.Quantity)) {
					item.AmountDisplay = fmt.Sprintf("%.2f%s", *ing.Quantity, unit.DisplayName)
				}
			} else if ing.UnitID == "suitable" {
				item.AmountDisplay = "适量"
			} else {
				item.AmountDisplay = unit.DisplayName
			}
		}
		// 库存比对：按基准量
		var requiredBase float64
		var iu models.IngredientUnit
		if config.DB.Where("ingredient_id = ? AND unit_id = ?", ing.IngredientID, ing.UnitID).First(&iu).Error == nil && ing.Quantity != nil {
			requiredBase = *ing.Quantity * iu.FactorToBase
		}
		var stockBase float64
		var items []models.IngredientItem
		q := config.DB.Where("ingredient_id = ?", ing.IngredientID)
		if familyID != "" {
		...
		for _, it := range items {
			var iu2 models.IngredientUnit
			if config.DB.Where("ingredient_id = ? AND unit_id = ?", it.IngredientID, it.UnitID).First(&iu2).Error == nil {
				stockBase += it.Quantity * iu2.FactorToBase
			}
		}
		if ing.UnitID == "suitable" && ing.Quantity == nil {
			if len(items) > 0 {
				item.Status = "sufficient"
			}
		} else if requiredBase <= 0 {
			item.Status = "unknown"
		} else if stockBase >= requiredBase {
			item.Status = "sufficient"
		} else {
			item.Status = "insufficient"
		}
		// 热量：基准量/100 * 每100克热量
		if nameOK && requiredBase > 0 {
			caloriesTotal += (requiredBase / 100) * master.CaloriesPer100
			proteinTotal += (requiredBase / 100) * master.ProteinPer100
			fatTotal += (requiredBase / 100) * master.FatPer100
			carbTotal += (requiredBase / 100) * master.CarbPer100
		}
		display = append(display, item)
	}
	return display, caloriesTotal, proteinTotal, fatTotal, carbTotal
}

// GetRecipeDetail 获取菜谱详情（含食材展示、库存状态、总热量）
func (h *RecipeHandler) GetRecipeDetail(c *gin.Context) {
	...
	if recipeType == "public" {
		...
		display, cal, pro, fat, carb := h.enrichRecipeDetail(recipe.Ingredients, userID, familyID)
		out := gin.H{
			"id": recipe.ID, "name": recipe.Name, "image": recipe.Image, "time": recipe.Time,
			"difficulty": recipe.Difficulty, "tags": recipe.Tags, "tagColors": recipe.TagColors,
			...
			"steps": recipe.Steps, "creatorId": recipe.CreatorID, "source": recipe.Source,
...
func (h *RecipeHandler) CreateRecipe(c *gin.Context) {
	...
	for _, ing := range req.Ingredients {
		if ing.IngredientID == "" || ing.UnitID == "" {
			continue
		}
		var iu models.IngredientUnit
		if err := config.DB.Where("ingredient_id = ? AND unit_id = ?", ing.IngredientID, ing.UnitID).First(&iu).Error; err != nil {
			c.JSON(http.StatusBadRequest, models.NewErrorResponse(
				models.CodeBadRequest,
				"食材「"+ing.IngredientID+"」的单位「"+ing.UnitID+"」不在常用单位中",
			))
			return
		}
	}
